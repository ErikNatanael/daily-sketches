...
fn update(app: &App, model: &mut Model, _update: Update) {
    ...
    if app.elapsed_frames() % 3 == 0 {
        let frame_num = app.elapsed_frames() as f32 * 0.3;
        let angle = frame_num / 10.0; 
        // Heart curve equation from here: https://mathworld.wolfram.com/HeartCurve.html
        let scale = (frame_num * 0.01).cos() * 80.0 + 100.0;
...
fn view(app: &App, model: &Model, frame: Frame) {
    ...
    let beating = (((app.elapsed_frames() as f32 * 0.03).sin() + 0.3) * PI).abs().sin().abs();
    ...
    draw.background().color(hsl(hue, 1.0, 0.5 - lightness));
    for p in &model.points {
        let pos = p.borrow().pos;
        draw.ellipse()
            .xy(pos * scale)
            .radius(3.0)
            .color(hsla(hue, 1.0, 0.2, 0.2));
        // draw lines
        for np in &p.borrow().lines {
            draw.line()
                .points(pos * scale, np.borrow().pos * scale)
                .color(hsla(hue, 0.95, 0.1 - lightness, 0.1))
                .weight(2.0);
        }
    }
